...
public class ManaAttributes {
    ...
    // Atributo de Mana Máxima (Padrão: 100.0, Mínimo: 0.0, Máximo: 1024.0)
    public static final RegistryObject<Attribute> MAX_MANA = ATTRIBUTES.register("max_mana",
            () -> new RangedAttribute("attribute.name.ebwplayermana.max_mana", 100.0D, 0.0D, 1024.0D).setSyncable(true));

    public static final RegistryObject<Attribute> MANA_REGEN = ATTRIBUTES.register("mana_regen",
            () -> new RangedAttribute("attribute.name.ebwplayermana.mana_regen", 2.0D, 0.0D, 100.0D).setSyncable(true));
